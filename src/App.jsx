...
// ── BASE PLAN ────────────────────────────────────────────
// exam_default = exam/target month
// start_default = study start month (overrides study_months calc if set)
const BASE_PLAN = [
  // Admission
  ...
  { id:"iia-literacy",  title:"IIA Data Literacy Certificate", cat:"audit",      type:"exam",      xp:50,  study_months:1,  start_default:"2027-02", exam_default:"2027-03", detail:"IIA · self-paced · updated Jun 2025 · member pricing" },
  { id:"iia-analytics", title:"IIA Data Analytics Certificate",cat:"audit",      type:"exam",      xp:50,  study_months:1,  start_default:"2027-03", exam_default:"2027-04", detail:"IIA · 16 CPE hours · includes Excel, SQL, Power BI, AI" },
...
const BADGES = [
  ...
  { id:"badge-alteryx",   icon:"⚙️", name:"Alteryx Certified",  desc:"Pass Alteryx Designer Core",          requires:["alteryx-core"] },
  { id:"badge-powerbi",   icon:"📊", name:"Power BI Pro",       desc:"Pass PL-300",                         requires:["pl300-exam"] },
  { id:"badge-audit",     icon:"🔍", name:"Audit Champion",     desc:"Complete all audit courses & certs",  requires:["cobit","coso","control","qaip","iia-literacy","iia-analytics"] },
  { id:"badge-togaf",     icon:"🏗️", name:"Architect",          desc:"Pass TOGAF Foundation",               requires:["togaf-exam"] },
  { id:"badge-reader",    icon:"📚", name:"Knowledge Base",     desc:"Complete all 8 readings",             requires:["read-iia-std","read-coso-ic","read-coso-erm","read-gtag","read-ca-guide","read-comp-gpg","read-da-gpg","read-acfe"] },
...
const buildCalendar = (plan, overrides) => {
  ...
  plan.forEach(p => {
    ...
    if (startMonth && startMonth < examMonth) {
      // generate study months between start and exam
      let cur = startMonth;
      let idx = 1;
      while (cur < examMonth) {
        entries.push({
...
const getAllMonths = (plan, overrides) => {
  ...
  plan.forEach(p => {
    ...
    if (startMonth) {
      let cur = startMonth;
      while (cur <= examMonth) { set.add(cur); cur = addMonths(cur,1); }
    }
  });
  return [...set].sort();
};
...
export default function App() {
  ...
  const getLoad = (monthId) => {
    const items = calendar.filter(p=>p.month===monthId);
    const exams = items.filter(i=>i.type==="exam"||i.type==="reading").length;
    if (items.length>=4||exams>=3) return { bg:"#FEE2E2", border:"#DC2626", label:"Heavy ⚠️" };
  ...
  if (!loaded) return (
    <div style={{ display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:"system-ui",flexDirection:"column",gap:12 }}>
      ...
      <div style={{ color:"#7C3AED",fontSize:13 }}>Loading…</div>
      ...
      <div style={{ maxWidth:860,margin:"0 auto",padding:"18px 14px" }}>
        ...
        {view==="calendar" && (<>
          <div style={{ display:"flex",gap:7,flexWrap:"wrap",marginBottom:16 }}>
            {[["all","All"],["exam","🎯 Exams"],["reading","📚 Readings"],["admission","🎓 Admission"],["cia","🏆 CIA"],["tech","🤖 Tech/AI"],["leadership","👑 Misk"],["audit","🔍 Audit"]].map(([id,l])=>(
        ...
        {view==="badges"&&(
          <div>
            <div style={{ fontSize:12,color:"#64748B",marginBottom:16 }}><strong style={{ color:"#7C3AED" }}>{BADGES.filter(b=>unlockedB(b)).length}/{BADGES.length}</strong> unlocked</div>
              ...
              {BADGES.map(b=>{
                ...
                return (
                  <div key={b.id} style={{ background:unlocked?"#fff":"#F8FAFC",border:`2px solid ${unlocked?"#7C3AED":"#E2E8F0"}`,borderRadius:14,padding:"16px 14px",textAlign:"center",boxShadow:unlocked?"0 4px 14px rgba(124,58,237,0.15)":"none" }}>
                    ...
                    {unlocked?<div style={{ fontSize:10,fontWeight:700,color:"#3B0764",background:"#EDE9FE",borderRadius:20,padding:"3px 10px",display:"inline-block" }}>✓ Unlocked!</div>:(
                      <div>
                        <div style={{ height:4,background:"#E2E8F0",borderRadius:4,marginBottom:4,overflow:"hidden" }}><div style={{ height:"100%",background:"#7C3AED",borderRadius:4,width:`${(progress/b.requires.length)*100}%` }}/></div>
        ...
        {view==="settings"&&(
          <div>
            <div style={{ fontSize:12,color:"#64748B",marginBottom:4 }}>
              Edit <strong>start date</strong> or <strong>exam month</strong> — calendar updates automatically.
            </div>
              ...
              return (
                <div key={catId} style={{ marginBottom:20 }}>
                  ...
                  {catItems.map(p=>{
                    ...
                    const modExam  = !!overrides[p.id];
                    const modStart = !!overrides[p.id+"_start"];
                    const modified = modExam || modStart;
                    ...
                    return (
                      <div key={p.id} style={{ background:"#fff",border:`1px solid ${modified?cc.border:"#E2E8F0"}`,borderLeft:`3px solid ${modified?cc.dot:"#E2E8F0"}`,borderRadius:10,padding:"10px 12px",marginBottom:8,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap" }}>
                        ...
                        <div style={{ display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
                          ...
                          {hasStudy && curStart && (
                            <div>
                              <div style={{ fontSize:9,color:"#94A3B8",marginBottom:2,textAlign:"center" }}>Study from</div>
                              <select value={curStart} onChange={e=>updateOverride(p.id+"_start",e.target.value)}
                                style={{ border:"1.5px solid #E2E8F0",borderRadius:7,padding:"5px 7px",fontSize:11,fontWeight:600,color:"#334155",background:"#F8FAFC",cursor:"pointer",outline:"none" }}>
                          ...
                          <div>
                            <div style={{ fontSize:9,color:"#94A3B8",marginBottom:2,textAlign:"center" }}>
                              {p.type==="exam"?"Exam":p.type==="milestone"?"Target":p.type==="reading"?"Read by":"Month"}
                            </div>
                            <select value={curExam} onChange={e=>updateOverride(p.id,e.target.value)}
                              style={{ border:`1.5px solid ${cc.border}`,borderRadius:7,padding:"5px 7px",fontSize:11,fontWeight:600,color:cc.text,background:cc.bg,cursor:"pointer",outline:"none" }}>
                              {monthOptions.map(m=><option key={m} value={m}>{ymLabel(m)}</option>)}
                            </select>
                          </div>
            ...
            <div style={{ marginTop:20,paddingTop:16,borderTop:"1px solid #E2E8F0",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              ...
              {Object.keys(overrides).length>0&&(
                <button onClick={()=>{setOverrides({});saveProgress(done,{});}} style={{ border:"1.5px solid #DC2626",background:"#FEE2E2",color:"#DC2626",borderRadius:8,padding:"7px 14px",cursor:"pointer",fontSize:12,fontWeight:600 }}>Reset all</button>
              )}
            </div>
          </div>
        )}
